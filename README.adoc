 ...
 ** https://github.com/apache/james-project/#run-james-with-java-8--guice--cassandra-rabbitmq-swift-elasticsearch-ldap[Run James with Java 8 + Guice + Cassandra + RabbitMQ + Swift + ElasticSearch + Ldap]
 ...
 * Java 8 + Guice + Cassandra + RabbitMQ + Swift + ElasticSearch + Ldap
 * Java 8 + Guice + Cassandra + ElasticSearch
 * Java 8 + Guice + JPA + Lucene
 * Java 8 + Spring + JPA


=== Run James with Java 8 + Guice + Cassandra + RabbitMQ + Swift + ElasticSearch + Ldap
...
If you haven't already:
  ...
  -v $PWD/dockerfiles/run/guice/cassandra-rabbitmq-ldap/destination:/cassandra-rabbitmq-ldap/destination \
...
You need a running *ElasticSearch* in docker. To achieve this run:
    ...
    $ docker run -d --name=ldap osixia/openldap:1.2.0
...
Then we need to build james container :
    ...
    $ docker build -t james_run dockerfiles/run/guice/cassandra-rabbitmq-ldap
...
To run this container :
   ...
   --link elasticsearch:elasticsearch --link tika:tika --link swift:swift --link ldap:ldap --name james_run -t james_run
